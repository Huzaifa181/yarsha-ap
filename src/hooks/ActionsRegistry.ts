...
export class ActionsRegistry {
  ...
  private lookupAction(url: string): RegisteredEntity | null {
    try {
  ...
  private lookupWebsite(url: string): RegisteredEntity | null {
    try {
      const urlObj = new URL(url);
      const host = urlObj.host;
  ...
  private lookupInterstitial(url: string): RegisteredEntity | null {
    try {
      const urlObj = new URL(url);
      const host = urlObj.host;
